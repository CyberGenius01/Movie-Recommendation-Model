...
def fetch_poster(movies):
    global movie_dict
    for movie in movies:
        if not movie[2] in movie_dict:
            movie_dict[movie[2]] = helper(movie)
        movie.append(movie_dict[movie[2]])
    print(movies)
    return movies
...
def helper(movie):
    try:
        req = requests.get(f"https://api.collectapi.com/imdb/imdbSearchById?movieId={movie[2]}", headers=headers)
        data = json.loads(req.text)
        link = data["result"]["Poster"]
    except KeyError:
        link = ('https://demofree.sirv.com/nope-not-here.jpg')
    return str(link)
